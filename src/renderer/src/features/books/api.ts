...
export type Book = ContentItem & { type: 'book' }
...
export async function listBooks(): Promise<BookListItem[]> {
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      item: toContentItem(itemRow) as Book,
      record: userRecords?.[0] ? toUserRecord(userRecords[0]) : null
    }
  })
}

export async function getBook(id: string): Promise<BookListItem | null> {
  const { data, error } = await supabase
    .from('content_items')
    .select('*, user_records(*)')
    .eq('id', id)
...
export async function updateBookDisplayOrder(id: string, displayOrder: number): Promise<void> {
  const { error } = await supabase
    .from('content_items')
...
export async function getArchivedAladinIds(): Promise<Set<string>> {
  const { data, error } = await supabase
    .from('content_items')
    .select('external_id')
    .eq('type', 'book')
