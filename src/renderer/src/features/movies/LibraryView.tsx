...
export function LibraryView({ onSelect, onCountChange, refreshKey }: Props): React.JSX.Element {
  ...
  const [orderPreview, setOrderPreview] = useState<MovieListItem[] | null>(null)
  ...
  async function commitReorder(): Promise<void> {
    ...
    if (!list) return
    const movedIndex = list.findIndex((m) => m.item.id === draggedItemId.current)
    ...
    try {
      await updateMovieDisplayOrder(movedId, nextOrder)
      setMovies((prev) =>
        prev.map((m) =>
