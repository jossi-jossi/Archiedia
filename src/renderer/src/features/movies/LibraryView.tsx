...
import { listMovies, updateMovieDisplayOrder, updateUserRecord, MovieListItem } from './api'
...
function MovieRow({
  ...
  item: MovieListItem['item']
  record: MovieListItem['record']
  reorderMode: boolean
  onSelect: (id: string) => void
  toggleStatus: (
    itemId: string,
    record: NonNullable<MovieListItem['record']>,
    tag: '보고 싶음' | '보는 중',
    active: boolean
  ) => void
}): React.JSX.Element {
...
export function LibraryView({ onSelect, onCountChange, refreshKey }: Props): React.JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted
  }, [movies, query, genreFilter, wishlistOnly, watchingOnly, sort])
  ...
  function handleDragEnd(event: DragEndEvent): void {
    ...
    const oldIndex = filtered.findIndex((m) => m.item.id === active.id)
    const newIndex = filtered.findIndex((m) => m.item.id === over.id)
    if (oldIndex === -1 || newIndex === -1) return
    const reordered = arrayMove(filtered, oldIndex, newIndex)
    const orderedValues = reordered
      .filter((_, i) => i !== newIndex)
      .map((m) => m.item.displayOrder ?? MISSING_ORDER)
    const nextOrder = computeDisplayOrderForInsert(orderedValues, newIndex)
    const movedId = active.id as string
    updateMovieDisplayOrder(movedId, nextOrder)
      .then(() => {
        setMovies((prev) =>
          prev.map((m) =>
            m.item.id === movedId ? { ...m, item: { ...m.item, displayOrder: nextOrder } } : m
          )
        )
      })
      .catch((err) => setError(errorMessage(err)))
