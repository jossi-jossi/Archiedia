...
function WebtoonRow({
  ...
  item: WebtoonListItem['item']
  record: WebtoonListItem['record']
  reorderMode: boolean
  onSelect: (id: string) => void
  toggleStatus: (
    itemId: string,
...
export function LibraryView({ onSelect, onCountChange, refreshKey }: Props): React.JSX.Element {
  ...
  function handleDragEnd(event: DragEndEvent): void {
    ...
    const oldIndex = filtered.findIndex((w) => w.item.id === active.id)
    const newIndex = filtered.findIndex((w) => w.item.id === over.id)
    ...
    updateWebtoonDisplayOrder(movedId, nextOrder)
      .then(() => {
        setWebtoons((prev) =>
          prev.map((w) =>
